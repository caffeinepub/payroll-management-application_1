...
export default function BulkMonthlyBankSalaryDialog({
  ...
  const handleSaveAll = async () => {
    try {
      ...
      // Prepare bulk data as { employeeId, month, year, amount }[]
      const bulkData: Array<{ employeeId: number; month: number; year: number; amount: number }> = [];
      ...
      for (const employee of employees) {
        ...
        if (salaryStr && salaryStr.trim() !== '') {
          ...
          if (amount > 0) {
            bulkData.push({ employeeId: employee.id, month, year, amount });
          }
        }
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Προσθέστε μηνιαίο μισθό τράπεζας για όλους τους εργαζομένους για τον επιλεγμένο μήνα.
          </DialogDescription>
        </DialogHeader>
